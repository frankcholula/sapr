...
if __name__ == "__main__":
    ...
    # feature_set = load_mfccs("eval_feature_set")
    # features = {word: load_mfccs_by_word("eval_feature_set", word) for word in vocabs}
    # total_features_length = sum(len(features[word]) for word in vocabs)
    # assert total_features_length == len(feature_set)
    ...
    # model = hmms["heed"]
    # mfcc = features["heed"][0]

    # print(f"MFCC shape before adjustment: {mfcc.shape}")
    # print(f"Model expected features: {model.n_features}")
    # log_prob, _ = model.decode(mfcc.T, algorithm="viterbi")
    # print(log_prob)